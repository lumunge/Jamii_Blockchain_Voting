...
contract ElectionFactory is IElectionFactory {
    ...
    function create_open_ballot(
        ...
        require(
            ...
            "You have reached the Max Limit of Ballots!"
        );
        ...
        for (uint256 i = 0; i < _ballot_candidates.length; i++) {
            new_ballot.ballot_candidates[i] = Candidate(
    ...
    function vote_open_ballot(address _candidate, uint256 _ballot_id) public {
        ...
        require(ballots.length <= _ballot_id, "Invalid Ballot Id!");
        require(
            !exists_address(ballot.ballot_candidates_addr, _candidate),
            "Candidate Does NOT exits in Ballot!"
        );
    ...
    function get_winner(uint256 _ballot_id) public payable returns (address) {
        ...
        for (uint256 i = 0; i < ballot_candidates.length; i++) {
            if (
