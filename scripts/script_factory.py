...
from scripts.utils import election_cost, ballot_cost, get_account
...
def deploy_election(_organization_name):
    account = get_account()
    new_election = JamiiFactory.deploy(_organization_name, {"from": account, "value": election_cost})
    print(f"Election deployed @ {new_election}")
    return new_election

def create_open_ballot(_ballot_name, _ballot_candidates_addr, _ballot_type):
    account = get_account()
    new_election = deploy_election("JAVA")
    new_ballot = new_election.create_open_ballot(_ballot_name, _ballot_candidates_addr, _ballot_type, {"from": account, "value": ballot_cost})
    print(f"Ballot Created @ {new_ballot}")
    return new_ballot
...
ballot_candidates = ["0xf9d48aC9eC8F207AEF93518B51D2CdA61e596904", "0x6c0A17AEe0a1420583446B77f0c8a55e369Bb07e"]
def main():
    deploy_election("Kenya")
    create_open_ballot("Ballot1", ballot_candidates, 0)
    # getBallotCandidates()
    # vote(testAccounts[0])
    # withdraw()